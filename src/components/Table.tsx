...
const Table = ({ patients, setPatients }) => {
  ...
  const moveToCurrent = () => {
    if (selectedPatientId !== null) {
      setPatients(
        patients.map((p) => (p.id === selectedPatientId ? { ...p, status: "current" } : p))
      );
    }
    closeModal();
  };
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="mb-6 bg-blue-100 p-4 rounded-lg">
        ...
        {patients.filter((p) => p.status === "upcoming").map((p) => (
          <div key={p.id} className="flex justify-between p-2 bg-white shadow rounded-lg my-2">
            ...
            <button onClick={() => openModal(p.id, "هل أنت متأكد من إلغاء الحجز؟")} className="bg-red-500 text-white px-3 py-1 rounded">
              إلغاء
            </button>
          </div>
        ))}
      ...
      <div className="mb-6 bg-yellow-100 p-4 rounded-lg">
        ...
        {patients.filter((p) => p.status === "waiting").map((p) => (
          <div key={p.id} className="flex justify-between p-2 bg-white shadow rounded-lg my-2">
            <span>{p.name} - {p.time} ({p.type})</span>
            <button onClick={() => openModal(p.id, "هل تريد بدء معالجة هذا المريض؟")} className="bg-green-500 text-white px-3 py-1 rounded">
              ابدأ المعالجة
            </button>
          </div>
        ))}
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title="تأكيد العملية">
        ...
        <div className="flex justify-center mt-4 space-x-4">
          <button onClick={modalContent.includes("إلغاء") ? cancelAppointment : moveToCurrent} className="px-4 py-2 bg-red-500 text-white rounded">
            نعم، تأكيد
          </button>
